 ...
 * @param {boolean} [includeExportDeclarations] Include local and re-export binding metadata.
 * @returns {LexedExports}
 */
export function lexEsm (moduleSource, parentURL, includeExportDeclarations = false) {
  ...
  for (const exported of exports) {
    ...
    if (exported.type === 'reexport-all') {
      ...
      if (includeExportDeclarations) {
        /** @type {ExportDeclaration} */
        const declaration = { type: exported.type, name }
        if (exported.type === 'direct') {
          if (typeof exported.localName === 'string') {
            declaration.localName = decodeExportName(exported.localName)
          }
        } else {
          declaration.specifier = exported.from
          if (typeof exported.importName === 'string') {
            declaration.importName = decodeExportName(exported.importName)
          }
        }
        exportDeclarations ??= new Map()
        exportDeclarations.set(name, declaration)
      }
    }
  }

  /** @type {LexedExports} */
  const result = { exportNames, starReexports, hasModuleSyntax }
  if (exportDeclarations !== undefined) result.exportDeclarations = exportDeclarations
  return result
}
