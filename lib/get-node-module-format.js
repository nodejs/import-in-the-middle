...
function getPackageFormat (extension, type) {
  if (type !== 'module' && type !== 'commonjs') return undefined
 ...
 * @param {boolean} [cachePackageTypes=true]
 * @returns {(url: string, packageJsonUrl?: string, packageType?: string) => NodeModuleFormat|undefined}
 */
module.exports = function createGetNodeModuleFormat (readFileSync, cachePackageTypes = true) {
  const packageTypes = cachePackageTypes ? new Map() : undefined
   ...
   * @returns {NodeModuleFormat|undefined}
   */
  return function getNodeModuleFormat (url, packageJsonUrl, packageType) {
    ...
    const visited = packageTypes === undefined ? undefined : []
