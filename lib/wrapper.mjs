...
`
  ...
  return `
import { registerWithData, ModuleBinder } from ${JSON.stringify(runtimeSpecifier)}
...
registerWithData(${JSON.stringify(realUrl)}, __binder, ${JSON.stringify(originalSpecifier)}, ${JSON.stringify(data)})
`
}
...
export function buildWrapperSourceWithData (options) {
  return buildESMWrapperSource(options)
}
