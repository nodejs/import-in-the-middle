...
import { commentShebang, sourceToString } from './source.mjs'
 ...
 * @param {string} options.realUrl The URL of the wrapped module.
 * @param {string[] | Map<string, string | StarBinding>} options.bindings Its exported bindings.
 * @param {string} options.originalSpecifier The specifier used to import the module.
 * @param {unknown} [options.data] Consumer data associated with the module.
 * @param {string} options.runtimeSpecifier The wrapper runtime import.
 * @param {(url: string) => string} [options.mapImport] Maps module URLs to bundler-owned imports.
 * @returns {string}
 */
...
`
  ...
  return `
import { register, ModuleBinder } from ${JSON.stringify(runtimeSpecifier)}
...
register(${JSON.stringify(realUrl)}, __binder, ${JSON.stringify(originalSpecifier)}, ${JSON.stringify(data)})
`
}
...
export function buildCommonJSWrapperSource ({
  ...
  source = commentShebang(sourceToString(source))
