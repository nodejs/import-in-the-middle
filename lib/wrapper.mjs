 ...
 */
function * resolveExportBinding ({ srcUrl, name, context, moduleExportsCache, memo, pending }) {
  ...
  try {
    ...
    if (declaration?.type === 'direct') {
      ...
      const target = yield * resolveBindingTarget(declaration.specifier, srcUrl)
 ...
 * @returns {Generator<Array, ResolvedExportBinding | undefined>}
 */
function * resolveStarExport ({ name, context, starReexports, moduleExportsCache, memo, pending }) {
  let binding
  for (const { specifier, parentURL } of starReexports) {
...
function * resolveBindingTarget (specifier, parentURL) {
  ...
  return yield [RESOLVE, request, { parentURL }]
}
...
export function buildCommonJSWrapperSource ({
  ...
  const argumentsList = preserveOuterBindings ? '' : ', module.exports, require, module, __filename, __dirname'
  return `(function (${parameters}) {${source}\n}).call(module.exports${argumentsList})\n` +
