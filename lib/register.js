...
const toHook = []
const registrySymbol = Symbol.for('import-in-the-middle:bundler-registry:v1')
const registry = globalThis[registrySymbol] ??= {
  extendedHooks: new Map(),
  toHookExtended: new Map()
}
const { extendedHooks, toHookExtended } = registry
 ...
 * @property {'module' | 'commonjs'} format
 * @property {{ exports: unknown }} [module]
 */
 ...
 * @returns {void}
 */
function registerWithData (name, binder, specifier, data) {
  const { namespace } = binder
  binders.set(namespace, binder)
  const proxy = new Proxy(namespace, proxyHandler)
  for (const hook of extendedHooks.values()) {
    hook(name, proxy, specifier, data, 'module')
  }
  toHookExtended.set(`module\0${name}`, { name, namespace: proxy, specifier, data, format: 'module' })
}

/**
 * @param {string} name The wrapped module URL.
 * @param {{ exports: unknown }} module Its CommonJS module object.
 * @param {string} specifier The original require specifier.
 * @param {unknown} [data] Consumer data associated with the module.
 * @returns {void}
 */
function registerCommonJS (name, module, specifier, data) {
  const entry = {
    name,
  ...
  for (const hook of extendedHooks.values()) {
    const replacement = hook(name, module.exports, specifier, data, 'commonjs')
    if (replacement !== undefined) module.exports = replacement
  }
  toHookExtended.set(`commonjs\0${name}`, entry)
}
...
class ModuleBinder {
   ...
   * @param {string[]} [passthroughKeys] Read-only export names that retain their source bindings.
   * @param {object[]} [passthroughSources] Alternate namespaces for read-only bindings.
   */
  constructor (source, keys, write, sources, passthroughKeys, passthroughSources) {
    this.#write = write
    if (keys !== undefined) {
...
exports.registerCommonJS = registerCommonJS
exports.registerWithData = registerWithData
exports.ModuleBinder = ModuleBinder
exports.extendedHooks = extendedHooks
exports.importHooks = importHooks
exports.specifiers = specifiers
exports.toHook = toHook
exports.toHookExtended = toHookExtended
