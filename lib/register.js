...
/**
 * @param {(name: string, exports: unknown, specifier: string, data: unknown,
 *   format: 'module' | 'commonjs') => unknown} hook
 * @param {HookEntry} entry
 * @returns {void}
 */
function applyHook (hook, entry) {
  const exports = entry.module === undefined ? entry.namespace : entry.module.exports
  const replacement = hook(entry.name, exports, entry.specifier, entry.data, entry.format)
  if (entry.module !== undefined && replacement !== undefined) {
    entry.module.exports = replacement
  }
}
...
function addHook (hook) {
  ...
  for (const entry of toHook) {
    applyHook(hook, entry)
  }
}
 ...
 */
function register (name, binder, specifier, data) {
  const { namespace } = binder
  specifiers.set(name, specifier)
  binders.set(namespace, binder)
  const proxy = new Proxy(namespace, proxyHandler)
  const entry = { name, namespace: proxy, specifier, data, format: 'module' }
