 ...
 * module.
 */
export function * getExports (url, context, includeExportDeclarations = false) {
  const useCache = context.cache !== false
  if (useCache) {
    const cached = esmExportsCache.get(url)
    if (cached !== undefined && (!includeExportDeclarations || cached.exportDeclarations !== undefined)) {
      return cached
    }
  }
  ...
  try {
    ...
    const { exportNames, exportDeclarations, starReexports, hasModuleSyntax } =
      lexEsm(source, url, includeExportDeclarations)
    /** @type {ModuleExports} */
    const moduleExports = starReexports === undefined ? { exportNames } : { exportNames, starReexports }
    if (exportDeclarations !== undefined) moduleExports.exportDeclarations = exportDeclarations

    if (moduleFormat === 'module') {
      if (useCache) esmExportsCache.set(url, moduleExports)
